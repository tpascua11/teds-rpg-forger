<template>
  <section class="">
    <section class="margin2">
			<div class="pure-u-14-24">
        <section class="">
          <div>
          </div>
					<div style="height: 32px;" class="">
            <div class="pure-u-6-24 this-title">
              {{reference}} list
						</div>
            <div class="pure-u-1-24"></div>
            <div class="pure-u-6-24 left">
              max
            </div>
            <div class="pure-u-1-24"></div>
            <div class="pure-u-6-24 left">
              current
						</div>
					</div>

					<div class="">
						<section class="" v-for="(item, index) in value.stat" :key="index">
							<div style="height: 35px;">
                <div class="pure-u-6-24">
                  {{list[index].name}}
                </div>
                <div class="pure-u-1-24"></div>
								<div class="pure-u-6-24" style="position: relative; bottom: 4px;">
									<input class="full-width writer" type="number" v-model="item.max" placeholder="num..">
                </div>
                <div class="pure-u-1-24"></div>
                <div class="pure-u-6-24" style="position: relative; bottom: 4px;">
									<input class="full-width writer" type="number" v-model="item.current" placeholder="num..">
                </div>
                <div class="pure-u-1-24"></div>
                <div class="pure-u-1-24">
                  <button v-on:click="cutConfirm(index)" class="pure-button
                    button-pink full-width"
                    style="height: 20px;"
                  >
										<div style="position: relative; right: 5px; bottom: 5px;"> X </div>
                  </button>
								</div>
							</div>
            </section>
            <br>
          </div>
        </section>
        <section>
          <section class="">
            <br>
            <div class="pure-u-8-24"></div>
            </section>
        </section>
      </div>
      <div class="pure-u-1-24"></div>
      <div class="pure-u-7-24">
        <div class=""> select new stat </div>
        <br>
        <SelectNameAndID v-model="test" :list="listNoRepeat"/>
        <button v-on:click="addStat(test)"
          class="pure-button full-width"
          style="height: 30px;">
          <div class="left b-font-2"> Add </div>
        </button>
      </div>
    </section>
  </section>
</template>


<script>

export default {
  name: 'List_Current_And_Max',
  components: {
  },
  data: function(){
    return {
      templateObj : [
        {hasStat: [{operator: ">"}] }
      ],
      template: {operator: ">"},
			flagList: Object.keys(this.$root.world.group.stat.list),
			list: this.$root.world.group.stat.list,
      keyList : Object.keys(this.$root.world.group.stat.list),
      test: {},
    }
  },
  props: ['value', 'reference', 'wel'],
  watch: {},
  mounted(){},
  methods:{
    addStat(stat){
      if(stat == {}) return;
      if(!stat.id) return;

      this.$set(this.value.stat, stat.id, {name: stat.name, max: 100, current: 0});
      this.test = {};
    },
    cut(list, index){
      list.splice(index, 1);
		},
		type(item){
			if(item.type != "%") item.type = "%";
			else item.type = "#";
			this.$forceUpdate();
		},
    cutConfirm(index){
      delete this.value.stat[index];
      this.$set(this.value, this.value);
			this.$forceUpdate();
    },
    closeModal(){
      this.$modal.hide('flagStatModal');
    },
    updateNow(){
		},
    updateID(){
    }
  },
  computed: {
    group: function(){
      return this.$root.world.group;
    },
    listNoRepeat: function(){
      let filteredArray = (Object.keys(this.list)).filter(
        function(e) {
          return this.indexOf(e) < 0;
        },
        [...Object.keys(this.value.stat)]
      );
      let filterObj = {};
      console.log("FILTERED ARRAY", filteredArray);
      filteredArray.forEach(index => {
        filterObj[index] = this.list[index];
      });
      console.log("FILTERED OBJ", filterObj);

      return filterObj;
    }
  }
}

</script>

<style scoped>
.this-title{
  font-size: 20px;
  text-decoration: underline;
  font-weight: bold;
}
.v-font{
  font-weight: bold;
	font-size: 13px;
	height: 22px;
	width: 100%;
}
.list-title{
  font-size: 17px;
	font-weight: bold;
	position:relative;
}
.v-select {
  font-size: 12px;
  font-weight: bold;
  width: 100%;
}
.referenceList{
  /* width: 200px; */
  width: 100%;
  height: 40px;
}
.b-font{
	color: darkred;
	position: relative;
	top: -5px;
}
.button-green{
	background-color: lightgreen;
}

.button-red {
	background-color: #ff6666;
}
.button-pink {
	background-color: pink;
}
.button-white{
	background-color: white;
	border: 1px;
}
.b-font-2{
	font-size: 15px;
}

.br-thin{
   display: block;
   margin: 2.5px;
 }

 .writer{
   font-size: 15px;
   height: 20px;
 }


</style>
