<template>
  <section class="" v-if="value.ifCondition == 'AREA'">
    <div class="">
      <div v-for="(item, index) in value.conditionList" class="" :key="index">
        <div class="col-12 col thin">
          <table class="table" style="table-layout: fixed;">
            ...
            <tbody v-if="item.areaIsList">
              <tr class="topcloser">
                <td width="10%"> {{index}}</td>
                <td> OR - Area Flag List -
                </td>
              </tr>
              <tr v-if="checkIfAndNot(item.areaIsList, item.areaNotList)">
                <td width="10%">  </td>
                <td>
                  Conflicted List
                </td>
              ...
              <tr class="">
                ...
                <td> <v-select v-model="item.areaIsList" :from="flagList" class="f-size adaptable-width" placeholder="Add Flag"> </v-select> </td>
              </tr>
              <tr>
                <td width="10%"> <p class="flagname"> NOT </p> </td>
                <td> <v-select v-model="item.areaNotList" :from="flagList" class="f-size adaptable-width" placeholder="Add Flag"> </v-select> </td>
              </tr>
              <tr>
                <td width="10%"> </td>
                <td>
                  ...
                  <button v-on:click="addNewList" class="btn-warning-outline btn-small smallfit btn-block">
                    Add New List
                  </button>
                </td>
              </tr>
...
export default {
  ...
  methods:{
    ...
    addNewList(){
      this.value.conditionList.push({areaIsList: [], areaNotList: []});
    },
    removeAtIndex(index){
  ...
  computed: {
    flagMap(){
      return Object.keys(this.$root.selectedArea.flagMap);
    }
  }
}
