  ...
  <div class="row big">
    <div class="col-2 paper" >
      ...
      <br>
        ...
        <v-select v-model="selected" as="name::name" :from="globalList" tagging
          @clearable="false"
          @select="selectGlobalEvent"
          @create="createNewGlobalEvent" class="">
        </v-select>
        ...
        <section class="interaction-list-scroll">
          <table class="table " style="width: 100%; table-layout: fixed;">
            <tbody>
              <tr class="thin-table-row" v-for="(item, index) in globalList" :key="index"
                v-on:click="selectInteraction(item)"
                v-bind:style="[ globalMatch(item) ?
                styleSelected : {}]"
              >
                <td class="thin-table-row moveable"> {{item}}</td>
              </tr>
            </tbody>
        ...
        <div class="row">
          <div class="col-4 col">
            <div class="row title3">
              ...
              <div class="col-1 col">
                <button v-on:click="rename()"
                  v-if="rememberName != selectedGlobalEvent.name"
                  class="btn-success-outline btn-small open-condition">
                    <i class="ra  ra-feather-wing  ra-lg"></i>
                    rename
                </button>
              </div>
            </div>
...
export default {
  ...
  data: function() {
    return {
      ...
      globalList: Object.keys(this.$root.world.globalEventMap),
      showCondition: true,
      selectedArea: {},
      rememberName: '',
    };
  ...
  methods:{
    ...
    selectInteraction(name){
      this.selected = name;
      this.selectedGlobalEvent = this.$root.world.globalEventMap[name];
      this.selectedGlobalEvent.name = name;
      this.rememberName = name;
    },
    selectGlobalEvent({value}){
      this.selectedGlobalEvent = this.$root.world.globalEventMap[value];
      if(this.selectedGlobalEvent){
        this.selectedGlobalEvent.name = value;
        this.rememberName = value.name;
      }
    },
    createNewGlobalEvent({value}){
      this.selectedGlobalEvent = this.$root.world.globalEventMap[value.name] = {
        name: value.name,
        scriptList: [],
        conditionList: [],
      };
      this.rememberName = value.name;
      this.globalList =  Object.keys(this.$root.world.globalEventMap);
    },
    globalMatch(name){
      return this.$root.world.globalEventMap[name] == this.selectedGlobalEvent;
    },
    addGlobal(){
    },
    ...
    rename(){
      if(this.rememberName == this.selectedGlobalEvent.name) return;
      this.$root.world.globalEventMap[this.selectedGlobalEvent.name] = this.selectedGlobalEvent;
      delete this.$root.world.globalEventMap[this.rememberName];
      this.rememberName = this.selectedGlobalEvent.name;
      this.globalList =  Object.keys(this.$root.world.globalEventMap);
    },
    //----------
    popShowCondition(){
